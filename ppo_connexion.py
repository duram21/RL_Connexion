...
class PPO(nn.Module):
    ...
    def v(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = F.relu(self.fc2(x))
        v = self.fc_v(x)
        return v.squeeze(-1)  # [batch] 또는 scalar
    ...
    def make_batch(self):
        ...
        done_mask = torch.tensor(np.array(done_lst), dtype=torch.float32, device=device)
        prob_a = torch.tensor(np.array(prob_a_lst), dtype=torch.float32, device=device)
    ...
    def train_net(self):
        ...
        with torch.no_grad():
            ...
            td_target = r + gamma * v_s_prime * done_mask.squeeze(-1)
            delta = td_target - v_s
        ...
        for k in range(T - 1, -1, -1):
            adv = delta[k] + gamma * lmbda * adv * done_mask[k].item()
            advantage_lst.append(adv)
        advantage_lst.reverse()
        advantage = torch.tensor(advantage_lst, dtype=torch.float32, device=device)
        ...
        for _ in range(K_epoch):
            pi = self.pi(s)  # [T, act_dim] 또는 [act_dim] → 위에서 squeeze 방지함
            if pi.dim() == 1:
                pi = pi.unsqueeze(0)

            pi_a = pi.gather(1, a).squeeze(1)  # [T]
            ratio = torch.exp(torch.log(pi_a + 1e-8) - torch.log(prob_a.squeeze(1) + 1e-8))
...
def test_run(model: PPO, num_episodes: int = 200):
    ...
    for ep in range(1, num_episodes + 1):
        ...
        # 점수 기준으로 승/패/무 (0 근처는 무승부로 볼 수도 있음)
        if total_reward > 0:
            wins += 1
        elif total_reward < 0:
            losses += 1
