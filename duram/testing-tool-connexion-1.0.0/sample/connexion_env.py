...
class ConnexionEnv:
    ...
    def get_smart_action_mask(self) -> np.ndarray:
        ...
        # 2. 필터링 규칙: 최고 점수의 70% 이상인 수만 허용
        # (만약 최고 점수가 0점이면, 그냥 모든 빈칸 허용)
    ...
    def step(self, action: int):
        ...
        # Smart Mask가 적용된 상태에서 Action이 들어옴
    ...
    def _opponent_random(self):
        ...
        for s in range(len(self.opp_hand)):
            for c in range(self.num_cells):
                if self.board[c] == -1: valid_moves.append((s, c))
        if valid_moves:
            s, c = valid_moves[self.rng.randint(len(valid_moves))]
